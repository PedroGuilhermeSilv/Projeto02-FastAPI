...
def test_update_product(db_session,product_on_db):
    ...
    product_update_on_db = db_session.query(ProductModel).filter_by(id=product_on_db.id)
...
def test_update_product_id_inavlid(db_session,product_on_db):
    product = Product(
        name='Roupa',
        slug='roupa',
        price= 22.2,
...
def test_list_products_search_uc(db_session,products_on_db):
    uc = ProductUseCases(db_session=db_session)
